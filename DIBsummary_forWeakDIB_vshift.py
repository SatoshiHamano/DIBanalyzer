...
if __name__ == "__main__":
    ...
    for o in objectID:
        cur.execute("SELECT x.combineID, x.measurementID, x.DIBID, x.echelleorder, x.DIBspecpath, "
                    ...
                    "where y.objectID=%d and z.category IN ('candidate');" % o)# NOT IN ('Cs I', 'weak');" % o) #y NOT IN ('Cs I', 'weak', 'fake', 'candidate');" % o)#
        rows = cur.fetchall()
                    ...
                    if EWset[repID][0] != 0.:
                        righttext = "  %s\n" % registeredname[dbID][0] + "   E(B-V)=%.2f\n" % ebv[dbID][
                            0] + "   Sp: %s\n" % \
                                    ...
                                    hv[repID][0]
                        if EWset[repID][0] < 5.*EWerrset[repID][0]:
                            print(registeredname[dbID][0], objIDax[n], EWset[repID][0], EWerrset[repID][0])
