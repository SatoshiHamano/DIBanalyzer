...
if __name__ == "__main__":
    ...
    cur.execute(
        "select objectID,E_BV,sptype,registeredname,radial_velocity from object join objectdict using(objectID) "
    ...
    for i in range(len(objectID)):
        radial_v[objectID[i]] = float(rows[i][4])
    ...
    for o in objectID:
        cur.execute("SELECT x.combineID, x.measurementID, x.DIBID, x.echelleorder, x.DIBspecpath, "
                    ...
                    "where y.objectID=%d and z.category NOT IN ('Cs I', 'weak', 'fake', 'candidate');" % o)
        rows = cur.fetchall()
    ...
    # wf2 = open("temporaly_files/DIBwavelength_update_20210422_2.txt", "w")

    for i in DIBset:#[65, 679, 79]:
        ...
        for j in range(3):
            ...
            for n in range(len(objIDax)):
                ...
                spxlist += [spx, spx, spx]
                spv = (spx - wav_air[i])/wav_air[i] * c - radial_v[objIDax[n]]
                spvlist += [spv, spv, spv]
                yshift_factor += [n, n, n]
            ...
            spectrum_plot(ax[j], spvlist, spyshifted, [-300, 300], [ylow, yupp],
                          colors=colorlist, yaxis_label="Normalized flux + offset",
                          lines=["--", "-", "-"], linew=[1., 1., 2.])
        ...
        r_dib, p_dib, _, rhol_ebv, rhou_ebv = DIBcorrelation(dibax, ew13175, ewerr13175, objectid13175, DIBwav13175, ewdict, ewerrdict,
                                      objIDpf, wav_air[i],
                                      stylefile=stylefile)
        fwhmax.hist([fwhmlist, fwhmnarrow], label=["Full sample", "Narrow sample"])
        fwhmax.set_xlabel(r'FWHM ($\AA$)')
        fwhmax.legend()
        Vcorrelation(vax, radial_v, objectID, DIBwav10780, heliovdict, objIDpfdet, wav_air[i],
                     stylefile=stylefile)
