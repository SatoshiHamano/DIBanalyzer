...
if __name__ == "__main__":
    ...
    pp = PdfPages(path + "Ref_%s.pdf" % ppidtel)
    plt.figure(figsize=(20, 20))

    for k in range(len(m_bef)):
        if m_bef[k] >= 42 and m_bef[k] <= 61:
            modelkey = modeltxt[m_bef[k]]
        else:
            modelkey = "INDEF"

        spx_b, spy_b, _, _, _ = openspecfits(spf_bef[k])
        spxlist, spylist = [spx_b], [spy_b]

        cur.execute("select y.telluricfilepath from telluriccorrection as x join telluricresult as y using(telluricID) where x.pipelineIDobj = '%s' and x.pipelineIDtel = '%s' and y.frame='%s' and x.telluricNumber=%d and y.echelleorder=%d;" % (ppidobj, ppidtel, frame, simnum, m_bef[k]))
        rows = cur.fetchall()
        spx_sim, spy_sim, _, _, _ = openspecfits(rows[0][0])
        spxlist_obj, spylist_obj = [spx_sim], [spy_sim]

        axtel = plt.axes([0.08, 0.1, 0.9, 0.4])

        MultiSpecAxis(axtel, spxlist, spylist, colors, xaxis_label="Wavelength", sptype="A", zerooffset=True, vacorair="VAC", C2plot=False, obsdates=obsdate, order=m_bef[k], model=modelkey)

        axobj = plt.axes([0.08, 0.55, 0.9, 0.4])

        MultiSpecAxis(axobj, spxlist_obj, spylist_obj, colors, sptype="OB", zerooffset=False, vacorair="VAC", C2plot=True, obsdates=obsdate, order=m_bef[k])
        plt.title("%s, %s, m=%d" % (ppidtel, ppidobj, m_bef[k]))
