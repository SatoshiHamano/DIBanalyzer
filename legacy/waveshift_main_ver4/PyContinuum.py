...
def PyContinuum(input, output, continuum_lowrej, continuum_highrej, continuum_order, continuum_func, continuum_sample):
    ...
    iraf.continuum(input, output, interactive="no", low_rej=continuum_lowrej, high_rej=continuum_highrej, grow=1., order=continuum_order, sample=continuum_sample, function=continuum_func)
...
if __name__ == "__main__":
    ...
    continuum_lowrej, continuum_highrej, continuum_order, continuum_func, continuum_sample = read_continuum_parameters(filename[2])

    if input.find("fits"):
        if len(continuum_lowrej) > 1:
            print "Too much continuum parameters for input list."
            sys.exit()
        PyContinuum(input, output, continuum_lowrej[0], continuum_highrej[0], continuum_order[0], continuum_func[0], continuum_sample[0])
    ...
    for i in range(len(inputl)):
        PyContinuum(inputl[i].split()[0], outputl[i].split()[0], continuum_lowrej[i], continuum_highrej[i], continuum_order[i], continuum_func[i], continuum_sample[i])
