...
def combineSpecFig(outputpdf, combineID, righttexts, rep_id, xmax=13400.):
    combinepath = obtainCombinePath(combineID[rep_id])
    datasetID, weight = obtainWeight(combineID[rep_id])
    maxid = weight.index(max(weight))
    obsdate = [datasetID[i][0:10] for i in range(len(datasetID))]
    vhelio = [read_rvfile_ID(datasetID[i]) for i in range(len(datasetID))]
